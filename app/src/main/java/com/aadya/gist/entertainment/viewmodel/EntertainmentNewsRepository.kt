...
class EntertainmentNewsRepository(application: Application) {
    private var application : Application

    init {
        this.application = application
    }
    private val entertainmentNewsLiveData = MutableLiveData<List<NewsModel>>()
    ...
    fun getEntertainmentNewsListState() : MutableLiveData<List<NewsModel>> {
        return  entertainmentNewsLiveData
    }
                        ...
                        try {
                            if (model?.statusCode == 200) {
                                if (response.body() != null) {
                                    ...
                                    entertainmentNewsLiveData.value = model.newsList
