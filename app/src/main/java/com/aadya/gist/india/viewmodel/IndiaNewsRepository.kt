...
class IndiaNewsRepository(application: Application) {
    private var application : Application

    init {
        this.application = application
    }
    private val indiaNewsLiveData = MutableLiveData<List<NewsModel>>()
    ...
    fun getIndiaNewsListState() : MutableLiveData<List<NewsModel>> {
        return  indiaNewsLiveData
    }
            ...
            RetrofitService().getNewsList(indiarequestModel,
                object : APIResponseListener {
                    override fun onSuccess(response: Response<Any>) {
                        ...
                        val indiamodel: ResponseModel? = response.body() as ResponseModel?
                        try {
                            if (indiamodel?.statusCode == 200) {
                                if (response.body() != null) {


                                    indiaNewsLiveData.value = indiamodel.newsList

                                }

                            } else if (indiamodel?.statusCode == 400) {
                                setAlert(
