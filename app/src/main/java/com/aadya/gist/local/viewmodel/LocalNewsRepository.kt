...
class LocalNewsRepository(application: Application) {
    private var application : Application

    init {
        this.application = application
    }
    private val localNewsLiveData = MutableLiveData<List<NewsModel>>()
    ...
    fun getLocalNewsListState() : MutableLiveData<List<NewsModel>> {
                        ...
                        try {
                            if (model?.statusCode == 200) {
                                if (response.body() != null) {
                                    ...
                                    localNewsLiveData.value = model.newsList
