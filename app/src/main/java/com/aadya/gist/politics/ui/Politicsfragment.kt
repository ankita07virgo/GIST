...
import com.aadya.aadyanews.databinding.FragmentPoliticsBinding
...
import com.aadya.gist.politics.adapter.politicsAdapter
import com.aadya.gist.politics.viewmodel.PoliticsNewsFactory
import com.aadya.gist.politics.viewmodel.PoliticsViewModel
import com.aadya.gist.registration.model.AlertModel
import com.aadya.gist.utils.*

class Politicsfragment : Fragment() {

    private lateinit var mBinding: FragmentPoliticsBinding
    private lateinit var politicsAdapter: politicsAdapter
    private lateinit var mViewModel: PoliticsViewModel
    ...
    override fun onCreateView(
        ...
        setUpRecyclerView(mBinding.iplRecyclerView)
        handleObserver()
        handleClickListner()
    ...
    private fun intializeMembers(inflater: LayoutInflater, container: ViewGroup?) {
        ...
        mViewModel = ViewModelProvider(
            this,
            PoliticsNewsFactory(activity?.application)
        ).get(PoliticsViewModel::class.java)
        mViewModel.getPoliticsNewsList(navigationmenu)
    }
    ...
    private fun handleObserver() {
        mViewModel.getPoliticsNewsListObserver().observe(viewLifecycleOwner, Observer {
            ...
            if (it.isEmpty()) {
                ...
                mViewModel.getPoliticsImagesList(navigationmenu)
                politicsAdapter.notifyData(it)
            }
        })
    ...
    private fun setUpRecyclerView(recyclerView: RecyclerView) {
        ...
        politicsAdapter = politicsAdapter(activity?.applicationContext,
            ArrayList<NewsModel>(),
            object : politicsAdapter.PoliticsListItemClick {
        ...
        recyclerView.adapter = politicsAdapter
    }
    ...
    private fun openDetailedNewsFragment(newsModelList: ArrayList<NewsModel>, recyclerview_position: Int) {
        ...
        ft?.replace(
            R.id.app_container,
            ViewPagerFragment.newInstance(newsModelList, 3, recyclerview_position)
