...
class PoliticsRepository(application: Application) {
    private var application : Application

    init {
        this.application = application
    }
    private val politicsLiveData = MutableLiveData<List<NewsModel>>()

    fun getPoliticsListState() : MutableLiveData<List<NewsModel>> {
        return  politicsLiveData
    }
                        ...
                        try {
                            if (model?.statusCode == 200) {
                                if (response.body() != null) {
                                    ...
                                    politicsLiveData.value = model.newsList
    ...
    private var  politicsimagesLiveData = MutableLiveData<List<Int>>()
    fun getPoliticsImagesListState() : MutableLiveData<List<Int>>{
        return  politicsimagesLiveData
    }

    fun getPoliticsImagesList(navigationmenu: NavigationMenu) {
        var  mImagesList = ArrayList<Int>()
        mImagesList.add(R.drawable.election1)
        mImagesList.add(R.drawable.election2)
        politicsimagesLiveData.value = mImagesList
    }
}
