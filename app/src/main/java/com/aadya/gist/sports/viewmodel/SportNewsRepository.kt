...
class SportNewsRepository(application: Application) {
    private var application : Application

    init {
        this.application = application
    }
    private val sportsNewsLiveData = MutableLiveData<List<NewsModel>>()
    ...
    fun getSportsNewsListState() : MutableLiveData<List<NewsModel>> {
                        ...
                        try {
                            if (indiamodel?.statusCode == 200) {
                                if (response.body() != null) {
                                    ...
                                    sportsNewsLiveData.value = indiamodel.newsList
