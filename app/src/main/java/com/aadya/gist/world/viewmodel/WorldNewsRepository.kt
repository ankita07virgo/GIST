...
class WorldNewsRepository(application: Application) {
    private var application : Application

    init {
        this.application = application
    }
    private val worldNewsLiveData = MutableLiveData<List<NewsModel>>()
    ...
    fun getWorldNewsListState() : MutableLiveData<List<NewsModel>> {
        return  worldNewsLiveData
    }
                        ...
                        try {
                            if (model?.statusCode == 200) {
                                if (response.body() != null) {
                                    ...
                                    worldNewsLiveData.value = model.newsList
